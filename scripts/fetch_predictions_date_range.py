"""
Fetch Predictions from Football API for a Date Range
Retrieves match predictions and odds from API-Football for a range of dates.

Usage:
    python scripts/fetch_predictions_date_range.py --start 2025-11-01 --end 2025-12-04
"""
import os
import sys
import json
...
class PredictionsFetcher:
    ...
    def fetch_for_date_range(self, league_id: int, start_date: str, end_date: str):
        """Fetch all predictions for a league within a date range."""
        ...
        print(f"\n🏆 Fetching predictions for {folder_name} from {start_date} to {end_date}")
        ...
        while current_date <= end:
            ...
            fixtures = self.fetch_fixtures(league_id, date_str)
            if not fixtures:
                current_date += timedelta(days=1)
                continue
            
            print(f"   📋 Found {len(fixtures)} fixtures on {date_str}")
            ...
            for fixture in fixtures:
                ...
                output_file = output_dir / f"{date_str}_{fixture_id}.json"
        ...
        print(f"   ✅ Complete. Found {total_fixtures} total fixtures. API calls: {self.calls_made}")


def main():
    parser = argparse.ArgumentParser(description="Fetch predictions from Football API for a date range")
    ...
    if args.all:
        ...
        for league_id in LEAGUE_FOLDERS.keys():
            fetcher.fetch_for_date_range(league_id, args.start, args.end)
    elif args.league:
        fetcher.fetch_for_date_range(args.league, args.start, args.end)
    else:
        print("Usage:")
        print("  python fetch_predictions_date_range.py --all --start 2025-11-01 --end 2025-12-04")
        print("  python fetch_predictions_date_range.py --league 39 --start 2025-11-01 --end 2025-12-04")
        print("\nLeague IDs:")
