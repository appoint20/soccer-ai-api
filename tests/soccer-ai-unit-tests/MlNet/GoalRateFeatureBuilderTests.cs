...
public class GoalRateFeatureBuilderTests
{
    ...
    public void ArtifactRejectsFeatureDriftAndDatesUsedForCalibration()
    {
        ...
        var artifact = new GoalRateArtifact { Trainer = "LightGbm", PredictionRecipe = GoalRateEnsemble.Recipe, Features = GoalRateRow.FeatureColumns(), DixonColes = dc,
        ...
        (artifact with { Trainer = "FastTreeTweedie" }).Supports(dc, hybrid).Should().BeFalse();
        (artifact with { Trainer = "" }).Supports(dc, hybrid).Should().BeFalse();
