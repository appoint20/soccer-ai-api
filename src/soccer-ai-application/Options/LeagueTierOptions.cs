...
public sealed class LeagueTierOptions
{
    ...
    /// <summary>
    /// England: 39 PL, 40 Championship, 41 League One, 42 League Two, 43 National League;
    ...
    /// </summary>
    /// <remarks>
    /// The National League is 43, confirmed against the provider
    /// (GET /leagues?country=England). It was once listed as 46 — which is the
    /// EFL Trophy, a cup — plus a placeholder 5 resolved by name at runtime.
    /// Both were removed on 2026-08-22 as unconfirmed, and the league itself
    /// stopped syncing with them, although 5 had been resolving to 43 all along.
    ///
    /// Configuration APPENDS to this default rather than replacing it (see
    /// <see cref="Services.LeagueTierService.GetSyncLeagueIds"/>), so a stale id
    /// left in a settings file survives a fix here. A new league also needs a
    /// name in <see cref="Services.LeagueCatalog"/> or the board labels it
    /// "League {id}".
    /// </remarks>
    public int[] Tier1 { get; set; } =
        [39, 40, 41, 42, 43, 78, 79, 80, 140, 141, 135, 136, 61, 62];
