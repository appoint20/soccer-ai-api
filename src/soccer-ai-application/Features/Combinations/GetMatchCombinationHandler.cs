    ...
    : IRequestHandler<GetMatchCombinationQuery, GetMatchCombinationResponse>
{
        ...
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            {
                logger.LogWarning("[Combinations] Gemini unavailable (Quota/Cap). Triggering Statistical Fallback...");
                // Fall through to the fallback logic below
            }

            // Step 3.1: STATISTICAL FALLBACK (If no combinations were generated by Gemini)
            if (combinations.Count == 0)
            {
                logger.LogInformation("[Combinations] No AI combinations found. Building statistical fallback from {Count} candidates.", matches.Count);
                
                // Use matches with high mathematical confidence (>= 75%)
                var statCandidates = matches
                    .Where(m => m.Prediction?.MatchWinner?.Confidence >= 0.75)
                    .OrderByDescending(m => m.Prediction?.MatchWinner?.Confidence ?? 0)
                    .ToList();

                if (statCandidates.Count >= 2)
                {
                    int comboId = 1;
                    foreach (var chunk in statCandidates.Chunk(2))
                    {
                        if (chunk.Length < 2) continue;
                        if (combinations.Count >= 3) break; // Limit to 3 fallback combos
                        ...
                        var combo = new CombinationDto
                        {
                            ...
                            Reason = "Top Statistical Advantage: These matches show a strong mathematical consensus based on historical form and scoring efficiency.",
                            Matches = chunk.Select(m => new CombinationMatchDto
                            {
                                ...
                                Reasoning = "High-confidence model prediction based on statistical advantage."
                            }).ToList()
                        };
