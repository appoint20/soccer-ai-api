    ...
    ILogger<PoissonCalculationService> logger) : IPoissonCalculationService
{
        ...
        CancellationToken ct)
    {
        ...
        return new LeagueAverages(
            leagueId,
            filteredMatches.Average(m => (double)m.HomeGoal),
            filteredMatches.Average(m => (double)m.AwayGoal),
        ...
        CancellationToken ct)
    {
        ...
        var homeMatchesRaw = await dbContext.Fixtures
            .Where(f =>
                f.LeagueId == leagueId &&
                f.HomeTeamId == teamId &&
                f.Status == "FT" &&
        ...
        var homeMatches = homeMatchesRaw
            .Where(f => f.Date < beforeDate)
            .OrderByDescending(f => f.Date)
            .ToList();

        var awayMatchesRaw = await dbContext.Fixtures
            .Where(f =>
                f.LeagueId == leagueId &&
                f.AwayTeamId == teamId &&
                f.Status == "FT" &&
        ...
        var awayMatches = awayMatchesRaw
            .Where(f => f.Date < beforeDate)
            .OrderByDescending(f => f.Date)
            .ToList();

        // Also fetch ALL matches (both home + away) for overall form (venue blending)
        var allMatchesRaw = await dbContext.Fixtures
            .Where(f =>
                f.LeagueId == leagueId &&
