    ...
    : IRequestHandler<GetUpcomingMatchesQuery, GetUpcomingMatchesResponse>
{
        ...
        IReceiveContext<GetUpcomingMatchesQuery> context, CancellationToken cancellationToken)
    {
        ...
        var fixturesQuery = dbContext.Fixtures.AsNoTracking();
        var total = await fixturesQuery.CountAsync(cancellationToken);
        ...
        {
            // Map Entity to DTO (Basic)
            var matchDto = new UpcomingMatchDto
            {
                Date = fixture.Date.ToString("yyyy-MM-dd"),
                Time = fixture.Time.ToString(@"hh\:mm"), // Check formatting
                HomeTeam = fixture.HomeName,
                AwayTeam = fixture.AwayName,
                League = fixture.LeagueName, // Name is already in Entity
                LeagueName = fixture.LeagueName
            };

            // Calculate Generic Stats
            var homeStats = await teamStatsService.CalculateStatsAsync(fixture.HomeName, allHistory);
            var awayStats = await teamStatsService.CalculateStatsAsync(fixture.AwayName, allHistory);
            
            // Calculate Advanced Analytics
            var advancedAnalytics = await advancedStatsService.CalculateAnalyticsAsync(fixture.HomeName, fixture.AwayName, allHistory);

            // True H2H
            var h2hMatches = await historicalRepository.GetMatchesBetweenTeamsAsync(fixture.HomeName, fixture.AwayName);
            var h2hAnalysis = CalculateHistoricalH2H(h2hMatches, fixture.HomeName, fixture.AwayName);

            // --- New Analytics (Last 9 / Last 3) ---
            
            // Home Team Data
            // Home Team Data
            var homeHistory = historicalRepository.GetMatchesForTeam(fixture.HomeName);
            var homeLast9 = homeHistory.OrderByDescending(m => m.Date).Take(9).ToList();
            var homeLast3Home = homeHistory
                .Where(m => string.Equals(m.HomeTeam, fixture.HomeName, StringComparison.OrdinalIgnoreCase))
                .OrderByDescending(m => m.Date)
                .Take(3)
                .ToList();

            var homeLast9Stats = teamAnalyticsService.CalculateStats(homeLast9, fixture.HomeName);
            var homeLast3HomeStats = teamAnalyticsService.CalculateStats(homeLast3Home, fixture.HomeName);

            // Away Team Data
            var awayHistory = historicalRepository.GetMatchesForTeam(fixture.AwayName);
            var awayLast9 = awayHistory.OrderByDescending(m => m.Date).Take(9).ToList();
            var awayLast3Away = awayHistory
                .Where(m => string.Equals(m.AwayTeam, fixture.AwayName, StringComparison.OrdinalIgnoreCase))
                .OrderByDescending(m => m.Date)
                .Take(3)
                .ToList();

            var awayLast9Stats = teamAnalyticsService.CalculateStats(awayLast9, fixture.AwayName);
            var awayLast3AwayStats = teamAnalyticsService.CalculateStats(awayLast3Away, fixture.AwayName);

            // Add to enriched list
            // Add to enriched list
            enrichedMatches.Add(matchDto with 
            { 
