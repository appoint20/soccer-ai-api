...
public static class AuditAiCombinedCommand
{
    public static async Task<int> RunAsync(string[] args)
    {
        ...
        foreach (var name in new[] { "fixtures", "ai-analyses", "prediction-snapshots", "model-forecasts" })
        {
            var path = Path.Combine(input, name + ".json");
            if (!files.TryGetProperty(name, out var entry))
            {
                if (name is not ("prediction-snapshots" or "model-forecasts") || File.Exists(path))
        ...
        var forecasts = await Read<ModelForecast>("model-forecasts");
        ...
        var report = new { GeneratedAtUtc = DateTimeOffset.UtcNow, InputProvenance = provenance.RootElement, Fixtures = fixtures.Count,
            ...
            RecordedAiForecasts = RecordedAiForecastStatistics.Build(fixtures, forecasts),
